...
echo -e "${YELLOW}1️⃣ . Дальше зайдите в директорию /root/gaianet/ и удалите 2 файла deviceid.txt, nodeid.json и вместо них закиньте свои старые файлы и поочерёдно киньте команды ниже!${NC}"
echo -e "${YELLOW}-------------------------------------------------------------------------------------------------------------------------------------------------------------------${NC}"
echo -e "${YELLOW}2️⃣ . source $HOME/.bashrc${NC}"
echo -e "${YELLOW}3️⃣ . gaianet init --config https://raw.githubusercontent.com/GaiaNet-AI/node-configs/main/qwen2-0.5b-instruct/config.json${NC}"
