...
echo -e "${YELLOW}Ждем 10 секунд пока нода запуститься и покажет порты...${NC}"
sleep 10

echo -e "${YELLOW}🔍 Вывод портов...${NC}"
sudo netstat -lntup | grep pop

echo -e "${GREEN}🚀 Установка PIPE завершена!${NC}"

echo "Если порты показало не корректно или вовсе не показало, то джем 1 мин и кидаем команду ниже"
echo "sudo netstat -lntup | grep pop"
